...
void scanline_convert( struct matrix *points, int i, screen s, zbuffer zb ) {
	...
	c.blue=rand()%255;
	c.green=rand()%255;
	double x0,x1,x2,y0,y1,y2,xb,xm,xt,yb,ym,yt,z0,z1,z2,zt,zbot,zm;
	x0=points->m[0][i];
	y0=points->m[1][i];
	z0=points->m[2][i];
	x1=points->m[0][i+1];
   ...
   double dz0 = (zt - zbot) / (yt - yb);
   double dz1 = (zm - zbot) / (ym - yb);
   double dz1_1 = (zt - zm) / (yt - ym);
	double z_0=zbot;
	double z_1=zbot;
   double x_0=xb;
   double x_1=xb;
   //printf("before while loop\n%lf %lf\n",x_0,x_1);
   	//printf("x values:%lf %lf %lf\n",x0,x1,x2);
	//printf("y values:%lf %lf %lf\n\n",y0,y1,y2);
   //draw_scanline((int)x_0,yb,z0,(int)x_1+4,yb,z1, s,zbot,c);
   while (yb<=ym){
	   	draw_scanline((int)x_0,yb,z_0,(int)x_1,yb,z_1, s,zb,c);
		x_0+=dx0;
		z_0+=dz0;
		if(abs(ym-yb)>0.5){
			x_1+=dx1;
	   ...
	   //printf("x0: %lf, x1: %lf, dx0: %lf, dx1: %lf\n\n",x_0,x_1,dx0,dx1);
...
void draw_polygons( struct matrix *polygons, screen s, zbuffer zb, color c ) {
  ...
  for (point=0; point < polygons->lastcol-2; point+=3) {
    ...
    if ( normal[2] > 0 ) {
	  ...
	  //printf("%lf %lf %lf %lf %lf %lf\n\n",polygons->m[0][point],polygons->m[1][point],polygons->m[0][point+1],polygons->m[1][point+1],polygons->m[0][point+2],polygons->m[1][point+2]);
	  //display(s);
